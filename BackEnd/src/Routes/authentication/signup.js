...
// ===== POST route =====
router.post('/', rateLimitMiddleware, upload.single('profilePicture'), (req, res, next) => {
  // Debug logging
  ...
  try {
      ...
      const emailSent = await sendVerificationEmail(newUser.email, newUser.userName, emailToken, newUser._id);
      
      if (!emailSent) {
        console.warn('Failed to send verification email, but user creation was successful');
      } else {
